...
2. Run the database builder (ensure you have Python installed):
```bash
python build_db.py
...
Create a new file named `.env` in the `bhagvadgpt-frontend` folder. Replace the Google keys with your own:
```env
# App Branding
APP_TITLE=BhagvadGPT

# API Lock (Disables OpenAI and forces the custom backend)
ENDPOINTS=custom

# Security Keys (Required for LibreChat to start securely)
...
1. Open your web browser (Incognito recommended for testing).
